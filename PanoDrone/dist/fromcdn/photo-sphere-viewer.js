...
(function (global, factory) {
  ...
  var AbstractAdapter = /*#__PURE__*/function (_AbstractService) {
    _inheritsLoose(AbstractAdapter, _AbstractService);

    function AbstractAdapter() {
      return _AbstractService.apply(this, arguments) || this;
    }
  ...
  var Renderer = /*#__PURE__*/function (_AbstractService) {
    ...
    _proto.transition = function transition(textureData, options) {
      ...
      this.setSphereCorrection(textureData.panoData, options.sphereCorrection, mesh); // rotate the new sphere to make the target position face the camera

      if (positionProvided) {
        var cleanPosition = this.psv.dataHelper.cleanPosition(options); // Longitude rotation along the vertical axis

        var verticalAxis = new THREE.Vector3(0, 1, 0);
        mesh.rotateOnWorldAxis(verticalAxis, cleanPosition.longitude - this.psv.prop.position.longitude); // Latitude rotation along the camera horizontal axis

        var horizontalAxis = new THREE.Vector3(0, 1, 0).cross(this.prop.direction).normalize();
      ...
      return new Animation({
        ...
        _this3.setSphereCorrection(textureData.panoData, options.sphereCorrection);
